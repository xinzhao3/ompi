...
//                                     remote_addr, rkey);
            opal_common_ucx_mem_post(module->state_mem,
...
//                             remote_addr, rkey);
    opal_common_ucx_mem_post(module->state_mem,
                             UCP_ATOMIC_POST_OP_ADD, (-1), target, sizeof(uint64_t),
                             remote_addr);
//    if (status != UCS_OK) {
...
//                                              mca_osc_ucx_component.ucp_worker);
        opal_common_ucx_mem_cmpswp(module->state_mem,
...
//                                       remote_addr, rkey, mca_osc_ucx_component.ucp_worker);
    ...
    opal_common_ucx_mem_fetch(module->state_mem,
...
//    ret = opal_common_ucx_ep_flush(ep, mca_osc_ucx_component.ucp_worker);
    opal_common_ucx_mem_flush(module->mem, OPAL_COMMON_UCX_SCOPE_EP, target);
    if (ret != OMPI_SUCCESS) {
        return ret;
    }
