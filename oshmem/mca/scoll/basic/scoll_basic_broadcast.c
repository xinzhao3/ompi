        ...
        for (i = 0; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) {
            ...
            if (pe_cur != PE_root) {
                SCOLL_VERBOSE(15,
                              "[#%d] send data to #%d",
                              group->my_pe, pe_cur);
                rc = MCA_SPML_CALL(put(SHMEM_CTX_DEFAULT, target, nlong, (void *)source, pe_cur));
            }
        }
    ...
    for (i = hibit + 1, mask = 1 << i; i <= dim; ++i, mask <<= 1) {
        ...
        if (peer_id < group->proc_count) {
            ...
            SCOLL_VERBOSE(14,
                          ...
                          group->my_pe, peer_pe);
            do {
                rc = MCA_SPML_CALL(get(SHMEM_CTX_DEFAULT, (void*)pSync, sizeof(long), (void*)pSync, peer_pe));
            } while ((OSHMEM_SUCCESS == rc) && (pSync[0] != SHMEM_SYNC_READY));

            SCOLL_VERBOSE(14, "[#%d] send data to #%d", group->my_pe, peer_pe);
            rc = MCA_SPML_CALL(put(SHMEM_CTX_DEFAULT, target, nlong, (my_id == root_id ? (void *)source : target), peer_pe));
