...
void shmem_integer_iput_f(FORTRAN_POINTER_T target, FORTRAN_POINTER_T source, MPI_Fint *tst, MPI_Fint *sst, MPI_Fint *len, MPI_Fint *pe)
{
    int i;
    ...
    for (i=0; i<length; i++)
    {
        MCA_SPML_CALL(put(SHMEM_CTX_DEFAULT, (uint8_t*)FPTR_2_VOID_PTR(target) + i * tst_c * integer_type_size,
            integer_type_size,
