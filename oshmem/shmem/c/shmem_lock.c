                                 ...
                                 int pe)
{
    ...
    if (target_size == 8) {
        MCA_ATOMIC_CALL(cswap(SHMEM_CTX_DEFAULT, target, (void*)&prev_value, (const void*)&cond, (const void*)&value, target_size, pe));
    } else if (target_size == 4) {
        uint32_t prev_value_32 = 0;
        uint32_t cond32 = (uint32_t) cond;
        uint32_t value32 = (uint32_t) value;

        MCA_ATOMIC_CALL(cswap(SHMEM_CTX_DEFAULT, target, (void*)&prev_value_32, (const void*)&cond32, (const void*)&value32, target_size, pe));
